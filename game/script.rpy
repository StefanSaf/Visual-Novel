...
label Lecture2:
    ...
    n "Most of you probably know that very very small object, like atoms, behave differently from the larger ones that we are familiar with."
    n "For example, an electron can be at two different places at once, whereas a melon can't. One common error is to think that we just don't know where exactly the electron is."
    n "That is partially true, but mostly wrong. We do not know where it is, because it really actually is at both places. But we can never see it in both places."
    n "But that is weird, isn't it? If something is, then it should be observable. Sadly, in the world of quantum mechanics everything is weird and different."
    n "When we observe an object in such a so called superposition, it seems to suddenly decide for one of the places randomly. We can calculate the probabilities for each event exactly."
    n "In fact, quantum mechanics is one of the most tested theories ever."
    ...
    n "But it doesn't explain what exactly an observation is. Does it need humans? Does consciosness play a role in the laws of physics? Or is a single photon hitting it enough?"
    n "This is the Copenhagen interpretation of quantum mechanics, and it leaves quite a few open questions. And yet, for a long time it was what most scientists thought was the correct way to see things."
    ...
    n "Now, let's think about what actually happens when we observe something. Let's take the most simple scenario, with a single electron."
    ...
    n "But the electron is in superposition! How does it reflect the electron? The answer is simple. The photon is reflected by both states of the electron, and is itself now in two positions at the same time."
    ...
    n "And when we look at the results of the detector, our whole brain is put into a superposition of thinking the electron is at either one place or the other. Both of these states are equally real."
    n "But because of how our brain works, we can only consciously feel ONE of them. If you want, you can call these other states parallel universes. But they are actually all part of one big, strange universe."
    n "And that is the Everett interpretation. If you ask me, it follows a much simpler logic and requires less magic. But of course, it also changes our view of the world completely."
    n "And extraordinary claims require extraordinary evidence. Can we produce such evidence? There are some theoretical possibilities. But that is a story for another day."
    n "I'm done here."
...
label go:
    scene boltzmann grab with slowfade
    show richard neutral at right
...
label following:
    ...
    stop music fadeout 2
    scene tu parking with slowfade
...
label true_end:
    ...
    "The walls... they don't feel so strong and ominous anymore. Volatile, almost."
